...
import styles from './subBtnBoard.module.css';

export default function SubBtnBoard({
  img = '',
  text = '',
  subBtnHandler = () => {},
  selectedBoardSid = -1, // 預設值為 -1，表示未選中任何看板
}) {
  ...
  // const isBoardSelected = selectedBoardSid === 1; // 修改為對應的看板索引
  
  // const fill = isClicked || isBoardSelected ? 'var(--light1)' : '#fff';
  const fill = isClicked || selectedBoardSid === 1 ? 'var(--light1)' : '#fff';

  return (
    <>
    <div className={`${styles.subBtn} ${isClicked ? styles.clicked : ''}`} onClick={handleClick}>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="333"
        height="316"
        fill={fill} // 使用 fill 變數
        viewBox="0 0 333 316"
