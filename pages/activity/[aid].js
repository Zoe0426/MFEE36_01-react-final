...
export default function ActivityDetail() {
  ...
  const [activitySidData, setActivitySidData] = useState([]);
  ...
  useEffect(() => {
    ...
    if (aid) {
      (async function getData() {
        //拿回特定活動資訊
        const res_activityInfo = await fetch(`${process.env.API_SERVER}/activity-api/activity/${aid}`,
          { method: 'GET' }
        );
        const {
          activitySidData
        } = await res_activityInfo.json();
